...
class Evaluator():
    ...
    def MI(cls, image_F, image_A, image_B):
        ...
        return max(skm.mutual_info_score(image_F.flatten(), image_A.flatten()), skm.mutual_info_score(image_F.flatten(),
                                                                                                   image_B.flatten()))
    ...
    def SSIM(cls, image_F, image_A, image_B):
        cls.input_check(image_F, image_A, image_B)
        data_range_F = image_F.max() - image_F.min()
        data_range_A = image_A.max() - image_A.min()
        data_range_B = image_B.max() - image_B.min()
        
        # Ensure data range consistency
        data_range = max(data_range_F, data_range_A, data_range_B)
        
        ssim_A = ssim(image_F, image_A, data_range=data_range)
        ssim_B = ssim(image_F, image_B, data_range=data_range)
        ssim_result = (ssim_A + ssim_B) / 2
        return ssim_result
