...
from net import Restormer_Encoder, Restormer_Decoder, HfFeatureExtraction, LfFeatureExtraction
...
BaseFuseLayer = nn.DataParallel(LfFeatureExtraction(dim=32, num_heads=8)).to(device)
DetailFuseLayer = nn.DataParallel(HfFeatureExtraction(in_c=32, out_c=32, num_layers=3)).to(device)
...
MSELoss = nn.MSELoss()  #MSE和L1都是回归loss
L1Loss = nn.L1Loss()
Loss_ssim = kornia.losses.SSIMLoss(11, reduction='mean')


# data loader
trainloader = DataLoader(H5Dataset(r"data/MoNuSeg_train_imgsize_256_stride_256.h5"),
...
if True:
    ...
    torch.save(checkpoint, os.path.join("models/IIFuse_v3_CDC3_FFC_ATTN8_INN3_"+timestamp+'_dim=32.pth'))
